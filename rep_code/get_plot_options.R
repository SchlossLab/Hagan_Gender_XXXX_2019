...
#preferred themes----
my_theme <- theme_classic() + #angled x-axis text, suppress legend
  theme(legend.position = "none", axis.text=element_text(size=12), axis.title=element_text(size=14,face="bold"), 
        plot.title = element_text(size=16,face="bold"), 
        axis.text.x = element_text(angle = 45, hjust = 1))

my_theme_horiz <- theme_classic() + #horizontal x-axis text, suppress legend
  theme(legend.position = "none", axis.text=element_text(size=12), axis.title=element_text(size=14,face="bold"), 
        plot.title = element_text(size=16,face="bold"))

my_theme_leg <- theme_classic() + #angled x-axis text, keep legend
  theme(axis.text=element_text(size=12), axis.title=element_text(size=14,face="bold"), 
        plot.title = element_text(size=16,face="bold"), 
        axis.text.x = element_text(angle = 45, hjust = 1))

my_theme_leg_left <- theme_classic() + #angled x-axis text, move legend to left of plot
  theme(axis.text=element_text(size=12), axis.title=element_text(size=14,face="bold"), 
        plot.title = element_text(size=16,face="bold"), 
        axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "left")

my_theme_leg_horiz <- theme_classic() + #horizontal x-axis text, keep legend
  theme(axis.text=element_text(size=12), axis.title=element_text(size=14,face="bold"), 
        plot.title = element_text(size=16,face="bold"))
...
gen_ed_facet <- function(x){
  ifelse(x == "female", "Women", "Men")
}
#geographic settings ------
geo_levels <- c("region", "GDP")
geo_linetype <- c("a", "g", "e", "f", "F1", "1F", "4C88C488")
geo_line_size <- c(1, 1.5, 2, 2.5, 0.5, 1.75, 2.25) 
...
#bar plot of total number of each region for the given time period
geographic_bar_plot <- function(df, group){

  plot <- ggplot(df)+
    geom_bar(aes(x = group, fill = group, group = group), position = "dodge", stat = "count")+
    #scale_fill_gradient(values = df, low = "#132B43", high = "#56B1F7", space = "Lab", na.value = "grey50", guide = "colourbar", aesthetics = "fill")+
    #scale_x_discrete(labels = gen_labels)+
    my_theme_horiz
...
#plot proportion of each region represented in a role over time, assumes df output from get_prop_by_yr()
geographic_line_plot <- function(df, ymax, group){

  plot <- ggplot(df) + 
    geom_line(aes(x = year, y = proportion, linetype = group, group = group))+
    coord_cartesian(ylim = c(0, ymax))+
    #scale_fill_manual(values = cbPalette)+
    #scale_linetype_manual(values = geo_linetype)+
    #scale_size_manual(values = geo_line_size)+
    theme(legend.position = "right")
  
  plot <- plot + my_theme_leg_horiz
