#plotting the break down of all submitting authors by region:
...
#setup ----
sub_df <- get(temp_sub) %>% 
  filter(GDP %in% gdp_list)#pull this df from the global environment
...
region_levels <- sub_df %>% pull(region) %>% unique()
...
unq_sub <- sub_df %>% 
  select(region, random.person.id) %>% distinct() %>% 
...
rbind(unq_pub, unq_sub) %>% 
  ggplot()+
  geom_bar(aes(x = region, fill = manu.type), position= "dodge")+
...
ggsave(filename = paste0(auth_type, "_total_subvpub_auth_geo.png"), path = "results/geographic/figures")

#print("barplot")
#break out author type by region and year for all journals combined ----
sub_authors_w_prop <- map_dfr(years, function(x){
  get_prop_by_yr(x, sub_df, "region", "All") %>% 
    mutate(., manu.type = "submitted")}) 

pub_authors_w_prop <- map_dfr(years, function(x){
  get_prop_by_yr(x, pub_df, "region", "All") %>% 
    mutate(manu.type = "published")}) 
...
text_values <- map_df(region_levels, function(x){
  all_authors_w_prop %>% 
    filter(region == x) %>% 
    get_gen_prop_text(., 2, "manu.type") %>% 
    mutate(region = paste(x))})

max_value <- get_ymax(all_authors_w_prop) 

#line plot of all journals combined by year
map(region_levels, function(x){  
  
  sub_y <- text_values %>% filter(region == x) %>% .[2,2]
  pub_y <- text_values %>% filter(region == x) %>% .[1,2]
  ...
  all_authors_w_prop %>% 
    ...
    my_theme_horiz + 
    labs(x = "Year",
         y = paste("Proportion of", x, auth_type, "Authors"),
         caption = paste("Proportion of", str_to_lower(x), 
                         str_to_lower(auth_type), 
                         "authors on manuscripts each year."))
  
  ggsave(filename = paste0(x, "_", auth_type, "_pub_v_sub_time.png"), 
         path = "results/geographic/figures")
 
  print(paste(x, "pub_v_sub"))
})  

#print("line plot by yr")
#
sub_j_authors_w_prop <- map_dfr(years, function(x){
  get_prop_by_yr(x, sub_df, "region", "Each") %>% 
    mutate(., manu.type = "submitted")})

pub_j_authors_w_prop <- map_dfr(years, function(x){
  get_prop_by_yr(x, pub_df, "region", "Each") %>% 
    mutate(., manu.type = "published")})
...
map(region_levels, function(x){
  ...
  ggsave(filename = paste0(x,"_", auth_type, "_j_geo_time.png"), 
         path = "results/geographic/figures")
  
  print(paste(x, "journ_pub_v_sub"))
})
