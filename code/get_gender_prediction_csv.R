...
#impact of variables on predicting the gender of the corresponding author: editor gender, reviewer gender, first author gender, last author gender, 1st round decision (e.g., editorial reject, post-review)
...
reg_data <- data %>% 
  select(role, published, doi, journal, num.versions, num.authors, 
         ...
         institution, US.inst) %>% 
  distinct() %>%
  mutate(corres.auth = if_else(author.corres == "TRUE", paste(gender), paste("NA")), #generate column for corresponding author gender
         editor = if_else(role == "editor", paste(gender), paste("NA")), #column for editor gender
         sen.editor = if_else(role == "senior.editor", paste(gender), paste("NA")) #column for senior editor gender
         ) %>% 
...
reviewers <- reg_data %>% 
  select(reviewer.gender, reviewer.random.id, grouped.random, 
         random.manu.num) %>% 
...
men_rev_data <- map_dfr(uniq.rev.manu, function(x){
  ...
  num_rev <- paste(c(summary[1,1], summary[2,1], summary[3,1]), 
                   collapse = "") %>% #pull reviewer count from summary
...
reg2_data <- full_join(corres_auth, editor, 
  ...
  distinct() %>% 
  mutate(editorial.reject = replace_na(editorial.reject, "no"),
