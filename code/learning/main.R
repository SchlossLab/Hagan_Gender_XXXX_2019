...
data <- data %>%
  select(-journal, -sen.editor.x.edreject, -edreject.gender, -US.inst, -US.inst.type, -inst.gender, -US.gender) %>% 
  drop_na() %>% 
  droplevels()
...
new_data <- dummy.data.frame(data, names=c("editor", "sen.editor","corres.auth"), sep=".")
