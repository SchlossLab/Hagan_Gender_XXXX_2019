...
pipeline <- function(dataset, model){
  ...
  inTraining <- createDataPartition(dataset$editorial.reject, p = .80, list = FALSE)
  ...
  print(model)
  trained_model <-  train(editorial.reject ~ ., # label
  ...
  # Here we look at the top 10 important features
    ...
    test_roc <- roc(ifelse(testing$editorial.reject == "yes", 1, 0), rpartProbs[[1]])
