...
#S5A. difference in editorial rejection by journal & inst type----
ed_rejs <- bias_data %>% 
  filter(published == "no") %>% 
...
Figure_S5A <- left_join(ASM_subs_j, ed_rej_subs_j, 
...
#S5B. Difference in accepted rates by journal & inst type----
acc <- bias_data %>% 
...
Figure_S5B <- left_join(ASM_subs_j, acc_subs_j, 
          by = c("US.inst.type", "gender", "journal")) %>% 
...
#S5C. Difference in review recommendation by institution type----
...
#S5D. difference in acceptance recommendation by reviewer gender----
...
plot_grid(Figure_S5A, Figure_S5B, Figure_S5C, Figure_S5D,
          labels = c('A', 'B', 'C', 'D'),
          label_size = 18,
          nrow = 2)
