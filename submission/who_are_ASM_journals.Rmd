...
source("../code/gender/editor_fig.R")
editor_A
editor_B
```       
  + Reviewer Notes:
...
```{r reviewer_gender}
source("../code/gender/reviewer_fig.R")
par(mfrow = c(1,3))
reviewer_A
reviewer_B
reviewer_C
...
source("../code/gender/summary_fig.R")
par(mfrow = c(1,3))
summary_A
summary_B
summary_C
...
#Discussion
  ...
  + Discuss literature on diversity and innovation - incentive to increase representation
well-documented that orgs w. more diverse & equitable representation are both more inclusive & supportive as well as more productive & innovative - Potvin, 2018
  + Suggestions to improve representation
      ...
      + Representational vs aspirational?
          + Largest & less visible workload (reviewing) should be representational
                  ...
                  + Be more familiar with international members of the field - suggest as reviewers
                  + Improve backend/interactivity of peer review selection
          + Higher visibility workload (editor, EiC) should be aspirational, recruit more women & individuals from international community
  + Future research directions
      + Study the possibility of bias during peer review based on gender/geography
      + Little evidence of gender discrimination in publication (C&W, 2011)
      + Studies have not found gender bias (with two exception) but have found geographic/language bias
